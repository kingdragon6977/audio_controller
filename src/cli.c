...
static void cli_history_recall(int direction)
{
    ...
    {
        if (history_pos < 0)
            history_pos = (int)history_count - 1;
...
static int run_i2s_capture(unsigned int sequence, int verbose)
{
    ...
    else
    {
        ...
        sprintf(buf,
                "  #%u A[min=%ld max=%ld mean=%ld peak=%ld] B[min=%ld max=%ld mean=%ld peak=%ld]\r\n",
                sequence,
                (long)min_a,
                (long)max_a,
                (long)(pairs ? sum_a / (int32_t)pairs : 0),
                (long)peak_a,
                (long)min_b,
                (long)max_b,
                (long)(pairs ? sum_b / (int32_t)pairs : 0),
                (long)peak_b);
        uart2_print(buf);
    }
...
static void execute(char *cmd)
{
    ...
    if (strcmp(cmd, "codec apply") == 0)
    {
        ...
        uart2_print(codec_apply_av6301_profile() ?
                    "Codec profile applied successfully.\r\n" :
...
void cli_task(void)
{
    while (uart2_available())
    {
        ...
        {
            esc_state = 0u;
            if (c == '~' && cursor_index < line_index)
            {
                for (i = cursor_index; i < line_index; i++)
                    line[i] = line[i + 1];
                line_index--;
                cli_redraw();
            }
        ...
        if (c == 0x08u || c == 0x7Fu)
        {
            if (cursor_index > 0)
            {
                for (i = cursor_index - 1; i < line_index; i++)
                    line[i] = line[i + 1];
                cursor_index--;
                line_index--;
                cli_redraw();
